...
start_steps = 10000      # 前1万步：只用fake（完美动作）填buffer
update_after = 1000      # buffer至少1000条经验后才能开始训练
batch_size = 100         # buffer至少100条才能采样（但代码里是 >= batch_size）
update_every = 50        # 每50步触发一次训练

## 阶段1：纯经验填充（t_total = 0 → 10000）
...
* `t_total = 10000`
* `memory.buffer_counter = 10000`（buffer里有10000条经验，全是用BFGS算的"标准答案"）

## 阶段2：混合生成 + 准备训练（t_total = 10000 → 10050）

开始用AI预测真实动作，但 **还没触发训练** （buffer已满，但等t_total%50==0）

**阶段2结束时：**

* `t_total = 10050`（多走了50步）
* `memory.buffer_counter = 10050`（但由于循环存储，只有10000条，index=0~9999）

## 阶段3：混合生成 + 训练触发（t_total = 10050

### 特点：**第一次训练！** 从此每50步训练一次

#### 经验生成总结

1. t_total不满足要求时,fakestep生成经验
2. t_total满足要求时,fakestep退出,agent介入.在满足t_total训练要求时,一次训练50次
...
环境的出生.输入结构,图转换函数,奖励计算函数,计算力函数

* **输入** ：用户给的原始结构列表 + 计算器
* **处理** ：用传统方法（BFGS）把每个结构弛豫到**局部能量最小点**
* **输出** ：`self.input_lib = {0: [标准答案结构, 计算器], 1: [...], ...}`

1. 用传统方法（BFGS）把每个结构**优化到平衡态**
2. `self.input_lib` = {0:[平衡态结构, 计算器], 1:[...], ...}

## reset

* **输入** ：`trans_coef`（打乱强度）、`num`（选哪个结构）、`correct`（是否安全检查）
* **处理** ：

1. 从 `input_lib`复制一份标准答案
2. 施加随机扰动（打乱）
3. **安全检查** ：如果原子太近（<r0），自动回撤

* **输出** ：返回 **被打乱的状态图** （s0）

1. 从 `input_lib`取出一个正确答案
2. trans_cong随机扰动
3. correct_rectify

## step

* **输入** ：`action`（AI预测的原子位移图）、`correct`（是否修正）
* **处理** ：

1. **执行动作** ：移动原子
2. **安全检查** ：检查距离，必要时回撤 → 得到 `a_back`
3. **更新状态** ：重新计算受力 → `next_state`
4. **判断收敛** ：力 < eps？→ `done`
5. **计算奖励** ：`-max_f` → `reward`
6. **停止信号** ：连续违规>20次？→ `stop`

* **输出** ：`(next_state, reward, done, action_corrected, max_f, stop)`

env.step(action)--> o2, r, d, a, max_f, s 新结构,奖励,是否收敛(游戏结束信号),修正后动作,最大力,连续违规信号
