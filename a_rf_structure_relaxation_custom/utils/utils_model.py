...
class Network(torch.nn.Module):
    r"""equivariant neural network adapted from e3nn MRS 2021 Fall Meeting Tutorial
    Parameters
    ----------
...
class Network_basic(torch.nn.Module):
    ...
    def get_convolution(self,irreps_in, irreps_hidden, irreps_edge_attr, irreps_node_attr, irreps_out, act, act_gates, layers, number_of_basis, radial_layers, radial_neurons, num_neighbors):
        ...
        for _ in range(layers):
            irreps_scalars = o3.Irreps([(mul, ir) for mul, ir in irreps_hidden if ir.l == 0 and tp_path_exists(irreps, irreps_edge_attr, ir)])
            irreps_gated = o3.Irreps([(mul, ir) for mul, ir in irreps_hidden if ir.l > 0 and tp_path_exists(irreps, irreps_edge_attr, ir)])
            ir = "0e" if tp_path_exists(irreps, irreps_edge_attr, "0e") else "0o"
            irreps_gates = o3.Irreps([(mul, ir) for mul, _ in irreps_gated])
            ...
            irreps = gate.irreps_out
            layers_list.append(CustomCompose(conv, gate))
