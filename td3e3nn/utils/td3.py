...
class TD3Agent:
    ...
    def __init__(self,
        ...
        self.tb_log_dir = None  # 将在train方法中设置
        self.writer = None
    ...
    def train(self, train_ep, test_ep, path_to_the_main_dir, env_name, test_every, start_iter = 0, save_every= 1000, e_lim = None, net_lim = None,
        ...
        """
        self.tb_log_dir = f"{path_to_the_main_dir}/runs/{env_name}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
        self.writer = SummaryWriter(log_dir=self.tb_log_dir)
        print(f"TensorBoard logs saved to: {self.tb_log_dir}")
        print(f"Run 'tensorboard --logdir={path_to_the_main_dir}/runs' to view")
        ...
        for i in range(train_ep[0]):
            ...
            for t in range(train_ep[1]):
                ...
                self.writer.add_scalar('Train/TotalReward_Step', ep_ret, t_total)
                self.writer.add_scalar('Train/Force_Step', f, t_total)
                self.writer.add_scalar('Train/Episode', i, t_total)
                self.writer.add_scalar('Train/Episode_Step', t, t_total)
                ...
                if t_total >= self.update_after and len(self.memory) >= self.batch_size and t_total % self.update_every == 0:
                    for j in range(self.update_every):
                        ...
                        if losses["loss_q"] is not None:
                            self.writer.add_scalar('Loss/Q', losses["loss_q"], t_total)
                        if losses["loss_pi"] is not None:
                            self.writer.add_scalar('Loss/Pi', losses["loss_pi"], t_total)
                ...
                if d or s or (t + 1 == train_ep[1]):
                    self.writer.add_scalar('Episode/Total_Reward', ep_ret, i)
                    self.writer.add_scalar('Episode/Length', ep_len, i)
                    self.writer.add_scalar('Episode/Stop_Reason', 1 if d else 0, i)
